...
void leerDato(std::string &tOrigen, std::string &tResultado, std::ifstream &ifs)
{
	...
	else 
	{
		...
		tOrigen = tOrigen.substr(1, tOrigen.size() - DIMENSIONES);
	}
}
...
void construyeMatriz(int totales[][DIMENSIONES], int * tOrigen, int * tResultado, const std::string tipoOriginal, const std::string tipoResultado)
{
	if(tipoOriginal == "SALINA.")
	{
		...
		if(tipoResultado == "NO CONDICIONADO") totales[0][2]++;
		if(tipoResultado == "CONDICIONADO.") totales[0][3]++;
	}
	if(tipoOriginal == "NO EMPAREJADO")
	{
		tOrigen[1]++;

		if(tipoResultado == "SALINA.") totales[1][0]++;
		if(tipoResultado == "NO EMPAREJADO") totales[1][1]++;
		if(tipoResultado == "NO CONDICIONADO") totales[1][2]++;
		if(tipoResultado == "CONDICIONADO.") totales[1][3]++;
	}
	if(tipoOriginal == "NO CONDICIONADO")
	{
		...
		if(tipoResultado == "NO CONDICIONADO") totales[2][2]++;
		if(tipoResultado == "CONDICIONADO.") totales[2][3]++;
	}
	if(tipoOriginal == "CONDICIONADO.")
	{
		tOrigen[3]++;

		if(tipoResultado == "SALINA.") totales[3][0]++;
		if(tipoResultado == "NO EMPAREJADO") totales[3][1]++;
		if(tipoResultado == "NO CONDICIONADO") totales[3][2]++;
		if(tipoResultado == "CONDICIONADO.") totales[3][3]++;
	}
	 
	if(tipoResultado == "SALINA.") tResultado[0]++;
	if(tipoResultado == "NO EMPAREJADO") tResultado[1]++;
	if(tipoResultado == "NO CONDICIONADO") tResultado[2]++;
	if(tipoResultado == "CONDICIONADO.") tResultado[3]++;
}
