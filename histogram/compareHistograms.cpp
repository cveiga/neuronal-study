...
int main(int argc, char *argv[])
{
	...
	int hisTraining[SIZE], hisTest[SIZE];
...
/**
* Lee un histograma
* @parametro miHistograma, vector de SIZE donde almacen los datos del histograma
...
std::string leerHistograma(int *miHistograma, std::string &img, std::ifstream &ifs)
{
	...
	{		
	    ...
	    ar & img & tipo;
	    for (int i = 0; i < SIZE; i++) { ar & miHistograma[i]; }
	}
	return tipo;
}
...
double distanciaEuclidea(int *hisTest, int *hisTraining, double &min, std::string &perteneceEuclidea, std::string tipoTraining)
{
	double resultado = 0;
	///////////////////////////////// DISTANCIA EUCLIDEA /////////////////////////////////////////////	
	for (int i = 0; i < SIZE; i++) resultado += pow(hisTest[i] - hisTraining[i], 2);			
